...
```shell
mamba create -n amst2-0.3.22-env -c bioconda -c conda-forge --override-channels python=3.11 "numpy<2.3" nibabel napari pyqt opencv "zarr>=3" vigra pandas snakemake=8
...
```shell
mamba create -n amst2-0.3.22-env -c bioconda -c conda-forge --override-channels python=3.11 "numpy<2.3" nibabel napari pyqt opencv "zarr>=3" vigra pandas snakemake=8 snakemake-executor-plugin-slurm=1
conda activate amst2-0.3.22-env
pip install SimpleITK-SimpleElastix transforms3d ruamel.yaml pyvsnr pytz tzdata
pip install https://github.com/jhennies/squirrel/archive/refs/tags/0.5.0.tar.gz
pip install https://github.com/jhennies/AMST2/archive/refs/tags/0.4.0.tar.gz
